...
def build_alert_payload(
    ...
    *,
    ...
    return {
        ...
        "startsAt": starts_at or _rfc3339_now(),
    }


def build_resolved_payload(payload, *, ends_at=None):
    """Build a resolution with exactly the firing alert's label identity."""
    resolved = copy.deepcopy(payload)
    resolved["endsAt"] = ends_at or _rfc3339_now()
    return resolved


def alert_delivery_succeeded(result):
    return result.startswith("Success:")


async def send_alert_payload(client, config, payload):
    """POST one pre-built firing or resolved payload with bounded retries."""
...
async def send_to_alertmanager(client, config, analysis, severity="info",
                               ...
                               resource="", incident_id="", starts_at=None):
    """Build and send one enriched firing alert to Alertmanager."""
    ...
    return await send_alert_payload(client, config, payload)
